...
`apps/notesprout_sn/RATTA_PLAN.md` § "Phases — Arc 24 \"Events\"" for the phase-by-phase record.
...
page with paper subsection below for what was built; `apps/notesprout_sn/HARVEST_PLAN.md` for the
...
**The statement order is what makes that true (arc 34 / M5).** `EventWrites` answers an
`EventWrite` in three parts, sent in this order: the **additions** (the event row's
`INSERT OR IGNORE` — a no-op on an existing event, first because the children's foreign key needs
it — then the note's additions, `NoteWrite.additions`: puts of the strokes this save minted, or the
whole copy under a new id), the **note's mutations** (`NoteWrite.mutations`: drops of loaded
strokes and re-puts of moved ones), and the **rewrites** last — the row's field update and its
three child sets, and at THIS / FOLLOWING scope the original's exception or truncation, the very
last statements of all. `EventWrite.batches` splits the additions on their own and keeps the
rewrites **whole in the last batch** (they ride the note mutations' last batch when they fit, else
a batch of their own), so a failure in any batch ahead of them leaves the original exactly as it
was, and a failure inside them is one transaction that lands nothing. Before M5 the original's
exception / truncation / field rewrite *led* the list, so a multi-batch write that failed later
left it mutated under a dialog that said nothing had changed. The note's mutations are the one
best-effort part past the cap: a batch of them that landed before a later batch failed stays landed
(the op log is still pending, so the next Save converges). `NoteWrite.inPlace` is what partitions
the op log by the minted set.
...
- **This and following** — `deleteWithScope` truncates the series to `UNTIL occurrence − 1`
  ...
  COUNT rule handed back unchanged from the editor's prefill carries the **remaining** count —
  the original's minus the starts ahead of the split (`Recurrence.countBefore`, exceptions still
  counting as spent slots), so "10 times" split at #5 is 4 + 6, not 4 + 10; a moved date is not a
  changed rule, but a rule the person retyped is theirs, count included (arc 34 / M3). The
  exceptions dated **at or after** the split carry over to the fresh series — the truncated
  part is the head, so an occurrence removed with THIS from the tail stays removed; a re-anchored
  tail carries them too, where they match nothing (arc 34 / M4). A
...
| An existing event's Save fails part-way | editor → store | same "Cannot save" dialog | **compensated**: the row and its children are kept — untouched, since the rewrite is the whole last batch (arc 34 / M5); only the strokes *this* save minted are dropped, one `DELETE` each |
| A THIS override or a FOLLOWING split fails part-way | editor → store | same "Cannot save" dialog | **compensated**: the minted row is deleted by id; the original series is untouched — its exception / truncation is the last statement of the last batch (arc 34 / M5) |
...
- `apps/notesprout_sn/HARVEST_PLAN.md` — arc 31 "Harvest"'s standalone plan and ledger; § D4/D5
  and the HV4/HV5 Outcome records are where this doc's § Export and Send-page-with-paper facts
  come from (not `RATTA_PLAN.md`).
- `apps/notesprout_sn/RATTA_PLAN.md` § "Phases — Arc 23 \"Calendar\"" — the wizard's locked
  decisions, the seam spec, and the Y1–Y4 Outcome records this doc draws its facts from.
- `apps/notesprout_sn/RATTA_PLAN.md` § "Phases — Arc 24 \"Events\"" — the events wizard's locked
