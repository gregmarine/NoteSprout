...
> **HISTORY (2026-09-11).** The effort is complete: Notesprout SN is the official Supernote version
> of Notesprout, merged to `main` (tag `notesprout-sn-0.1.0`), folder renamed `apps/notesprout_ratta`
> → `apps/notesprout_sn`, version `0.1.0-ratta` → `0.1.0-sn`, branch `ratta` deleted. This file and
> the per-arc `<ARC>_PLAN.md` files are the ledger of *why*; the working protocol is now the
> maintenance protocol in `apps/notesprout_sn/CLAUDE.md`. Nothing below is resumed.

**Branch:** `ratta` (merged to `main`, deleted) · **Location:** `apps/notesprout_sn/` · **Package:** `com.symmetricalpalmtree.notesproutsn`
**Label:** Notesprout SN (debug: "Notesprout SN Dev") · **Version:** `0.1.0-ratta` (now `0.1.0-sn`)
...
walk logs) live in git history — arcs 1–18 at `git show 90a9198:apps/notesprout_sn/RATTA_PLAN.md`,
...
`0.1.0-ratta`; **2087 JVM tests/variant**. **Arc 25 "Drive" (the EIGHTH point, `ACTION_CLOUD_STORAGE`, granted 2026-09-04; V1–V6 complete + frozen 2026-09-05 — `:ext-cloud`, THIRTEEN modules, `API_VERSION` 8, 2340 JVM tests/variant, reference `docs/cloud.md`) lives in its OWN file — `DRIVE_PLAN.md`; read that, not this, for arc-25 work.** **Arc 26 "Keys" (og-parity encryption, `PARITY_BACKLOG.md` item 1; U1–U7 complete + frozen 2026-09-05 — host-only, no point, no API bump, 1077 `:app` tests, reference `docs/encryption.md`) lives in its OWN file — `ENCRYPTION_PLAN.md`; read that, not this, for arc-26 work. Arcs 1–26 are all frozen.** **Arc 27 "Restore" (whole-library restore, `PARITY_BACKLOG.md` item 2; L1–L6 complete + frozen 2026-09-06 — host-only, no point, no API bump, 1194 `:app` tests, reference `docs/restore.md`) lives in its OWN file — `RESTORE_PLAN.md`; read that, not this, for arc-27 work. Arcs 1–27 are all frozen. **Arc 28 "Objects" (sticky notes, text objects, six hand-placed shapes — `PARITY_BACKLOG.md` item 3; H1–H7 complete + frozen 2026-09-06 — host-only, no point, no API bump, g-paper re-pinned 0.1.23 → 0.1.27 for the transform mode, 1470 `:app` tests / 2830 across the modules, code review waived by the user, reference `docs/objects.md`) lives in its OWN file — `OBJECTS_PLAN.md`; read that, not this, for arc-28 work. Arcs 1–28 are all frozen.** **Arc 29 "Loop" (the lasso eraser, `PARITY_BACKLOG.md` item 4; LE1–LE4 complete + frozen 2026-09-07 — an engine tool `Tool.LASSO_ERASER` in g-paper 0.1.28 (re-pinned from 0.1.27) armed from an eraser re-tap Point · Lasso sub-bar on all four paper surfaces; host + `:sn-screen` + `:ext-ink`, no point, no API bump, no new row, 1472 `:app` tests / 2832 across the modules, no code review, reference `docs/notebook.md`) lives in its OWN file — `LOOP_PLAN.md`; read that, not this, for arc-29 work. Arcs 1–29 are all frozen.** **Arc 30 "Page" (page erase + page export, `PARITY_BACKLOG.md` item 5; PE1–PE3 complete + frozen 2026-09-08 — an Erase page row (one soft-delete transaction over `liveDescendantIds`, `Action.PageErased` by id) and an Export page row (close → `ExportActivity` at page scope → reopen; `ExportScope`'s host-side page filter, Soil hidden at page scope, `ExportSpec` + every exporter untouched); host-only, no point, no API bump, no new row, 1487 `:app` tests / 2847 across the modules, no code review, references `docs/notebook.md` + `docs/export.md`) lives in its OWN file — `PAGE_PLAN.md`; read that, not this, for arc-30 work. Arcs 1–30 are all frozen.** **Arc 31 "Harvest" (export and import extras, `PARITY_BACKLOG.md` item 6; HV1–HV6 complete + frozen 2026-09-08/09 — `:ext-image` PNG exporter with per-page folder delivery (FOURTEEN modules), Save as template, `export_preset` index rows, `ICalendar.render` + `outgoingTarget` at `API_VERSION` **9** (two compatible tails, no floor moved) for calendar file export and a papered page on send; no ninth point, no code review, 1564 `:app` tests / 2945 across the modules, references `docs/export.md` + `docs/calendar.md` + `docs/templates.md` + `docs/notebook.md` + `docs/extensions.md`) lives in its OWN file — `HARVEST_PLAN.md`; read that, not this, for arc-31 work. Arcs 1–31 are all frozen.** **Arc 32 "Resume" (launch restore, `PARITY_BACKLOG.md` item 7 — the LAST item; RS1–RS3 complete + frozen 2026-09-09 — a surface stack in prefs (`sn_view_state` / `surfaceStack`) maintained from lifecycle by the host screens and from `open`/`onResult` by the extension entries, a pure `ReplayPlan` the library runs on a cold launch that reopens the whole chain (notebook → calendar / pad / document editor, the calendar → pad pair, or a library-level calendar / pad) with every missing target dropped; host-only, no point, no API bump, no schema change, no code review, 1606 `:app` tests / 2987 across the modules, references `docs/library.md` § Launch restore + `docs/notebook.md` § Cold-launch restore) lives in its OWN file — `RESUME_PLAN.md`; read that, not this, for arc-32 work. Arcs 1–32 are all frozen and `PARITY_BACKLOG.md` is closed.** **Arc 33 "Focus" (a single-finger double-tap hides / shows all chrome on the four paper screens, full-page calendar grids, full-bleed sticky editor — a fresh user decision; F1–F5 complete + frozen 2026-09-09 — `:sn-screen` `ChromeBand` / `ChromeToggle` + the `rectOf` visibility rule, host `ChromePrefs` + `DoubleTapToggleRule`, `EXTRA_CHROME_HIDDEN` as a compatible tail both ways, `CalendarGeometry` without insets and `CalendarBars` deleted; no point, no API bump, no schema change, no code review, 1626 `:app` tests / 3033 across the modules, references `docs/notebook.md` + `docs/calendar.md` + `docs/scratchpad.md` + `docs/objects.md` + `docs/sn-screen.md` + `docs/extensions.md`) lives in its OWN file — `FOCUS_PLAN.md`; read that, not this, for arc-33 work. **Arc 34 "Prune" (the 32 confirmed findings of the 2026-09-09 `/code-review` of arcs 24–33 — 1 high, 9 medium, 22 low — fixed; a fresh user decision; P1–P4 complete + frozen 2026-09-10 — H1 read-only staged-store verify so a local backup carrying a `-wal` restores; M1–M9 rotation / events / notebook / extension-launch correctness + the one-bind per-page PNG export and the Drive folder-id cache; L1–L22 reuse, dead code, conventions; no point, no API bump, no schema change, no g-paper change, no new module, no code review of the fixes; 1654 `:app` tests / 3078 across the modules after the 2026-09-10 post-freeze L17 fix — the raw key must reach `ATTACH … KEY` as TEXT; every fix's as-built shape in its ledger, four refuted candidates listed — do not re-raise) lives in its OWN file — `PRUNE_PLAN.md`; read that, not this, for arc-34 work. Arcs 1–34 are all frozen.** **Arc 35 "Halves" (a Day's whole-page Send carries both halves, AM then PM, as two papered pages in one undo step — a fresh user decision 2026-09-10; HA1–HA2 complete + frozen 2026-09-10 — `ICalendar.advanceOutgoing` at `API_VERSION` **10** (method floor, no action floor moved), the `parkCompanionPages` hook, `CalendarSession`'s outbound FIFO, `drainFurtherPages` + `onDrained(List)`, `Action.PagesReceived`, and a "Receiving from …" box over every send; no point, no schema change, no code review, 1654 `:app` tests / 3079 across the modules, references `docs/calendar.md` + `docs/extensions.md` + `docs/notebook.md`) lives in its OWN file — `HALVES_PLAN.md`; read that, not this, for arc-35 work. Arcs 1–35 are all frozen.** **Arc 36 "Corner" (arc 33's hidden chrome collapses to a corner tool button with a mini toolbar and a mirrored overflow row on all four paper screens — a fresh user decision 2026-09-11; C1–C3 complete + frozen 2026-09-11 — `:sn-screen` `CollapsedChrome` + pure `CollapsedTools`, `ChromeToggle(whileHidden, beforeShow)`, `AnchoredBar.show(anchor)`; no point, no API bump, no schema change, `/code-review high` at the freeze with its findings fixed, 1662 `:app` tests / 3097 across the modules, references `docs/notebook.md` + `docs/sn-screen.md` + `docs/scratchpad.md` + `docs/calendar.md` + `docs/objects.md`) lives in its OWN file — `CORNER_PLAN.md`; read that, not this, for arc-36 work. Arcs 1–36 are all frozen.** No other arc and no NINTH extension point without another user decision.**
...
1. **One phase per session.** At phase start: read this file, root `CLAUDE.md`, and
   `apps/notesprout_sn/CLAUDE.md`. Confirm the next `⬜` phase with the user, flip it to
...
- **Own Gradle root** at `apps/notesprout_sn/`. Gradle 8.14, AGP 8.11.1, Kotlin 2.2.20,
...
- **g-paper pin: 0.1.28** (arc 29 / LE1 2026-09-06 — `Tool.LASSO_ERASER`; 0.1.27 through arc 28, 0.1.23 through arc 27) in `sn-screen/build.gradle.kts` — `gpaper-core` + `gpaper-ratta`
...
- **Standing rules** live in `apps/notesprout_sn/CLAUDE.md` (read it every phase start).
...
## Frozen arcs — ledger (full records: `git show 90a9198:apps/notesprout_sn/RATTA_PLAN.md`)
...
1. All JVM unit tests green (`./gradlew test` in `apps/notesprout_sn`).
...
| Frozen arc phase records | `git show 90a9198:apps/notesprout_sn/RATTA_PLAN.md` |
...
### Z5 — UI cleanup ✅ (**wizard locked 2026-09-03**; Z5a 2026-09-03 · Z5b 2026-09-04 — the user's five items + one Fable candidate; nothing else)
...
card is flush at the band's bottom; header side padding 20 dp lines the label up with the badge
column. Editor bar is `activity_events.xml`'s FrameLayout idiom (title first, 96 dp side margins,
buttons last). `CalendarTemplate.note()` = a page-sized ARGB bitmap carrying only `bandLabel` at
(0, 0) — transparent ground like `month()`; `NoteSurface` takes a `bakeTemplate` lambda, bakes in
`applyPageSize` only when the size changes, recycles the old after `setTemplate`, and recycles the
last in `release()`. The text half is a `noteTextHalf` wrapper (label + `inputNote`) and
`applyKind` toggles the wrapper. `Editor.Latch` is the one latch style; `Editor.Weekday` extends it
with no items. `LatchGroup<T>` (`pressed` / `resolve`, unknown selection → FIRST down, never none)
+ 7 tests. Walk: cards with regular weight + black meta, "Today"/"Upcoming" kept · Cancel left /
title centred / Save right · "Notes" on the paper template and above the text field · Ends
`[Never][On a date][After]` with the date button / count stepper under the row, dialog narrows and
re-centres under the IME (cosmetic — a Dialogs.style window, expected). Crash buffer empty.
...
**Z5b — the two controls (Opus on a Fable brief, Fable review) ✅ e77cc160 2026-09-04 (outcome below the list):**
...
**Z5b outcome ✅ (2026-09-04, Opus on a Fable brief, Fable review, Fable walked the Nomad by hand;
2072 → 2087 JVM tests/variant):** all three controls landed as specified. **Pill toggle** —
`:sn-screen`'s `Widget.Notesprout.Toggle` (an `AppCompatCheckBox`, `button=@null`, the whole look is
`toggle_pill`, a `state_checked` selector of two fixed-size layer-lists; **the style's 56 dp width is
the drawable's geometry** — the 9 dp knob insets are computed from it, change both or neither; the
view is `toolbar_button_size` tall so the 44 × 24 dp pill sits in a real tap target); the editor's
`swAllDay` keeps its id and its `CompoundButton` listener logic untouched. **Clock face** — pure
`ClockFaceModel` (twelve positions for both faces, clockwise from the top; `hit` = `atan2(dx, -dy)`
rounded to 30°, null inside 0.30 r and outside 1.15 r) + `ClockFaceView` (1 dp rim, labels at 0.78 r,
the picked number a 20 dp inverted disc, `ACTION_UP` picks, no animation, 240 dp desired through
`resolveSize`); `dialog_time_picker` = `[9]:[00]` hour/minute LATCHES (the showing face is down and
tapping either is the way back), the dial, AM/PM. Picking an hour swaps to minutes by itself; picking
a minute stays. `TimeMath.stepHour`/`stepMinute` deleted (the rest is untouched — still the time
truth). **Count latches** — pure `CountPresets` (`pressed`/`moreLabel`: More is down above 6 and
reads the number) + `KeypadModel(range, current)` (leading zero replaced, digit count capped at the
range's width, `value()` clamps, empty = keep `current`) + `CountLatches` (the seven-button wrapper,
`view_count_latches` `<include>`d three times) + `KeypadDialog` (`1 2 3 / 4 5 6 / 7 8 9 / ⌫ 0 ✓` —
**Done is the ✓ key in the grid**, the dialog bar carries Cancel alone; `ic_backspace` NEW in
`:sn-screen`, Tabler). Repeat's "every" row is a sentence line (`EventWording.repeatGlance` — one
wording with the glance button) over the latches; the count is "N times" (`editor_times_n` /
`editor_times_one`) over its own row; Remind keeps its preview, latches, then Days/Weeks on their own
line. `EventDraft.withIntervalValue` / `withCountValue` are the one clamp; the delta forms delegate
(no production caller left — kept for the tests). **Deleted as dead:** `Editor.StepperValue`,
`Editor.Inline`, `RepeatDialog.unitLabel`, the eight `editor_unit_*` strings, `editor_times`, the six
`cd_editor_*_minus/plus` and four `cd_time_*` stepper strings. `docs/sn-screen.md` icon count was
already stale at 55 → now 56 and named. **Walk (Nomad, by hand, all green):** pill ON/OFF both ways
and the time buttons follow · clock at 9 inverted → tap 2 → minutes face → 35 → PM → OK = "2:35 PM"
on the editor · Weekly: seven latches fit the dialog width with room, More → keypad "Every" title,
typed 12 → ✓ → "Every 12 weeks" + the More latch reads "12" down · After → "10 times" row with 10
down → preset 3 → "3 times" → More reads "More" again · Remind: 2 + Weeks → "2 weeks before" ·
Discard clean; crash buffer empty. **Cosmetic, noted not fixed:** at interval 1 the sentence line
reads "Weekly" directly under the dialog's "Weekly" title (it is `repeatGlance`'s word — one wording
by design; "Every 2 weeks" the moment it is not 1). **Trap (new):** the auto-mode permission
