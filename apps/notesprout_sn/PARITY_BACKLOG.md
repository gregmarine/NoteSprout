# PARITY_BACKLOG.md — the og-parity work still wanted in Notesprout SN (CLOSED 2026-09-11 — every item done; the `ratta` branch merged to `main`)
...
review of `apps/notesprout_android` (og Notesprout) against `apps/notesprout_sn` (Notesprout SN)
...
**Read before starting any of them:** the root `CLAUDE.md`, `apps/notesprout_sn/CLAUDE.md`, and
...
as a bug (`apps/notesprout_sn/CLAUDE.md`, `docs/scratchpad.md`). Shared ink helpers live in
