plugins {
    id("com.android.application")
    id("org.jetbrains.kotlin.android")
}

android {
    namespace = "com.symmetricalpalmtree.notesproutsn.ext.image"
    compileSdk = 35

    defaultConfig {
        applicationId = "com.symmetricalpalmtree.notesproutsn.ext.image"
        minSdk = 29
        targetSdk = 35
        versionCode = 1
        // Host lockstep (N0 wizard): same stamp as :app, bumped together at arc freezes.
        versionName = "0.1.0-ratta"
    }

    buildTypes {
        debug {
            applicationIdSuffix = ".dev"
            versionNameSuffix = "-dev"
            // The dev extension serves the dev host.
            buildConfigField("String", "HOST_PACKAGE", "\"com.symmetricalpalmtree.notesproutsn.dev\"")
        }
        release {
            isMinifyEnabled = false
            buildConfigField("String", "HOST_PACKAGE", "\"com.symmetricalpalmtree.notesproutsn\"")
        }
    }

    buildFeatures {
        buildConfig = true
    }

    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }

    kotlinOptions {
        jvmTarget = "17"
    }
}

dependencies {
    // The whole of it: the contract library, and the framework's own PNG encoder. A raster format
    // needs no third-party assembler — :ext-pdf's module-local pdfbox has no twin here.
    implementation(project(":extension-api"))
    testImplementation("junit:junit:4.13.2")
}
